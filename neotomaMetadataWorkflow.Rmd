...
The collectionunits table is the place to go to learn about core-specific metadata, including precise core location. In order to access the collectionunits associated with our Tanganyika sites, we first need to download the entire collectionunits table. Then we'll filter for the collection units we care about. Below, we grab the collectionunits table as a list, reformat it into a dataframe, and then an sf object, and plot it on top of our map.
