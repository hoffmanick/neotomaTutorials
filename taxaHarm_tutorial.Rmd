...
justvals_all = wide_data_all %>% ungroup() %>% dplyr::select(!c(contactname,contactid,status))

distances_all = dist(justvals_all, method = 'euclidean')


agglomerative_clustering_all <- hclust(distances_all, method = 'average')



colors = as.data.frame(agglomerative_clustering_all$order) 

names(colors) = c("order")

colors = colors %>%  dplyr::mutate(color = case_when(order <= 30 ~ "blue", order > 30 ~ 'red'))
agglomerative_clustering_all$labels = wide_data_all$contactname

fviz_dend(agglomerative_clustering_all, cex = 0.5, show_labels=TRUE,label_cols = colors$color,k=10,color_labels_by_k = TRUE)
...
pcaplot = ggplot(wide_data_all) + 
  geom_point(mapping=aes(x=pc1,y=pc2,color=status,group=contactname)) +
  geom_text(mapping=aes(x=(pc1+0.5),y=(pc2-0.5),label=contactname,color=status)) +
